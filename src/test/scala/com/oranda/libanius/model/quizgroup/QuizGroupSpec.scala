...
class QuizGroupSpec extends Specification with AppDependencyAccess {
  ...
  "a quiz group" should {
    ...
    "move an existing quiz pair to the front of its queue" in {
      ...
      val foundQuizItem =
        QuizItemSource.produceQuizItem(qgUpdated.levels.head, CurrentPromptNumber(qgUpdated.currentPromptNumber))
    ...
    val memLevel2 = QuizGroupMemoryLevel(
      correctResponsesInARow = 2,
      repetitionInterval = 15,
      totalResponses = 2,
